...
pub enum Statement {
    ...
    DefStruct { span: Span, name: String, props: Vec<String> },
}
...
impl TokenStream {
    ...
    fn expect(&mut self, test: impl Fn(&TokenValue) -> bool) -> PResult<Token> {
        match self.peek() {
            Some(token) if test(&token.value) => {
                Ok(self.next().expect("Checked"))
            },
...
fn parse_while(tokens: &mut TokenStream) -> PResult<Expr> {
    let span = tokens.expect(|t| matches!(t, TokenValue::While)).map_err(|_| ParseError::Continue)?.span;
...
fn parse_loop(tokens: &mut TokenStream) -> PResult<Expr> {
    let span = tokens.expect(|t| matches!(t, TokenValue::Loop)).map_err(|_| ParseError::Continue)?.span;
    let _ = tokens.expect(|t| matches!(t, TokenValue::LBrace))?;
    let body = parse_body(tokens);
    let _ = tokens.expect(|t| matches!(t, TokenValue::RBrace))?;
...
fn parse_pipe(tokens: &mut TokenStream) -> PResult<Expr> {
    ...
    while let Ok(p) = tokens.expect(|t| matches!(t, TokenValue::Pipe)) {
        ...
        left = Expr::Pipe { span, parent: left.into(), child: right.into() };
    }
    Ok(left)
}
...
fn parse_andor(tokens: &mut TokenStream) -> PResult<Expr> {
    ...
    while let Ok(op) = tokens.expect(|t| matches!(t, TokenValue::And | TokenValue::Or)) {
        ...
        left = Expr::Binary { span, op: op.value, left: left.into(), right: right.into() };
    }
    Ok(left)
}
...
fn parse_comparison(tokens: &mut TokenStream) -> PResult<Expr> {
    ...
    while let Ok(op) = tokens.expect(|t| matches!(t, TokenValue::Eq | TokenValue::Ne | TokenValue::Gt | TokenValue::Lt | TokenValue::Gte | TokenValue::Lte)) {
        let span = op.span;
...
fn parse_muldiv(tokens: &mut TokenStream) -> PResult<Expr> {
    ...
    while let Ok(op) = tokens.expect(|t| matches!(t, TokenValue::Star | TokenValue::Slash | TokenValue::Coef)) {
        let span = op.span;
...
fn parse_assignment(tokens: &mut TokenStream) -> PResult<Expr> {
    ...
    while let Ok(bind) = tokens.expect(|t| matches!(t, TokenValue::Assign | TokenValue::Semicolon | TokenValue::Mutate)) {
        let span = bind.span;
...
fn parse_self(tokens: &mut TokenStream) -> PResult<Expr> {
    let token = tokens.expect(|t| matches!(t, TokenValue::_Self)).map_err(|_| ParseError::Continue)?;
    Ok(Expr::Term(token.span, token.value))
}

fn parse_identifier(tokens: &mut TokenStream) -> PResult<Expr> {
    let token = tokens.expect_identifier().map_err(|_| ParseError::Continue)?;
    Ok(Expr::Term(token.span, token.value))
}

fn parse_if(tokens: &mut TokenStream) -> PResult<Expr> {
    let span = tokens.expect(|t| matches!(t, TokenValue::If)).map_err(|_| ParseError::Continue)?.span;
...
fn parse_for(tokens: &mut TokenStream) -> PResult<Expr> {
    let span = tokens.expect(|t| matches!(t, TokenValue::For)).map_err(|_| ParseError::Continue)?.span;
    ...
    Ok(Expr::For { span, pin, iterable, body })
}
    ...
    loop {
        match tokens.peek() {
            Some(token) if matches!(token.value, TokenValue::Dot) => {
                ...
                let field = Expr::Term(
                    next.span,
                    TokenValue::Str(next.value.identifier()),
                );
                target = Expr::Access { span, target: target.into(), field: field.into() };
            },
            Some(token) if matches!(token.value, TokenValue::LParen) => {
                ...
                target = Expr::Call { span, target: target.into(), args };
            },
            Some(token) if matches!(token.value, TokenValue::LBracket) => {
                ...
                target = Expr::Access { span, target: target.into(), field: index.into() };
            }
            _ => break,
        }
    };
...
fn parse_grouped(tokens: &mut TokenStream) -> PResult<Expr> {
    let _ = tokens.expect(|t| matches!(t, TokenValue::LParen)).map_err(|_| ParseError::Continue)?.span;
...
fn parse_nil(tokens: &mut TokenStream) -> PResult<Expr> {
    let token = tokens.expect(|t| matches!(t, TokenValue::Nil)).map_err(|_| ParseError::Continue)?;
    Ok(Expr::Term(token.span, token.value))
}

fn parse_collection(tokens: &mut TokenStream) -> PResult<Expr> {
    let span = tokens.expect(|t| matches!(t, TokenValue::LBrace)).map_err(|_| ParseError::Continue)?.span;
...
fn parse_bool(tokens: &mut TokenStream) -> PResult<Expr> {
    let token = tokens.expect(|t| matches!(t, TokenValue::True | TokenValue::False)).map_err(|_| ParseError::Continue)?;
    Ok(Expr::Term(token.span, token.value))
}

fn parse_return(tokens: &mut TokenStream) -> PResult<Expr> {
    let span = tokens.expect(|t| matches!(t, TokenValue::Return)).map_err(|_| ParseError::Continue)?.span;
    let value = Box::new(parse_expression(tokens)?);
    Ok(Expr::Return(span, value))
}

fn parse_break(tokens: &mut TokenStream) -> PResult<Expr> {
    let span = tokens.expect(|t| matches!(t, TokenValue::Break)).map_err(|_| ParseError::Continue)?.span;
    let value = Box::new(parse_expression(tokens)?);
    Ok(Expr::Break(span, value))
}

fn parse_unary(tokens: &mut TokenStream) -> PResult<Expr> {
    let op = tokens.expect(|t| matches!(t, TokenValue::Minus | TokenValue::Not | TokenValue::Star)).map_err(|_| ParseError::Continue)?;
    let span = op.span;
    let value = Box::new(parse_term(tokens)?);
    Ok(Expr::Unary { span, op: op.value, value })
}

fn parse_continue(tokens: &mut TokenStream) -> PResult<Expr> {
    let span = tokens.expect(|t| matches!(t, TokenValue::Continue)).map_err(|_| ParseError::Continue)?.span;
    Ok(Expr::Continue(span))
}

fn parse_struct_create(tokens: &mut TokenStream) -> PResult<Expr> {
    let span =  tokens.expect(|t| matches!(t, TokenValue::New)).map_err(|_| ParseError::Continue)?.span;
    ...
    Ok(Expr::Struct { span, name: name.into(), props })
}

fn comma_separated<T>(mut f: impl FnMut(&mut TokenStream) -> PResult<T>) -> impl FnMut(&mut TokenStream) -> Vec<T> {
    move |tokens| {
...
fn parse_sequence(tokens: &mut TokenStream) -> PResult<Expr> {
    let span = tokens.expect(|t| matches!(t, TokenValue::LBracket)).map_err(|_| ParseError::Continue)?.span;
...
fn parse_float(tokens: &mut TokenStream) -> PResult<Expr> {
    let token = tokens.expect(|t| matches!(t, TokenValue::Float(_))).map_err(|_| ParseError::Continue)?;
    Ok(Expr::Term(token.span, token.value))
}

fn parse_int(tokens: &mut TokenStream) -> PResult<Expr> {
    let token = tokens.expect(|t| matches!(t, TokenValue::Int(_))).map_err(|_| ParseError::Continue)?;
    Ok(Expr::Term(token.span, token.value))
}

fn parse_string_literal(tokens: &mut TokenStream) -> PResult<Expr> {
    let token = tokens.expect(|t| matches!(t, TokenValue::Str(_))).map_err(|_| ParseError::Continue)?;
    Ok(Expr::Term(token.span, token.value))
}
...
fn parse_call(tokens: &mut TokenStream) -> PResult<Expr> {
    ...
    while let Ok(t) = tokens.expect(|t| matches!(t, TokenValue::LParen)) {
        ...
        let args = parse_args(tokens);
        let _ = tokens.expect(|t| matches!(t, TokenValue::RParen))?;
        left = Expr::Call { span, target: left.into(), args };
    }
    Ok(left)
}
...
fn parse_func(tokens: &mut TokenStream) -> PResult<Expr> {
    let span = tokens.expect(|t| matches!(t, TokenValue::Fn)).map_err(|_| ParseError::Continue)?.span;
    ...
    Ok(Expr::Func { span, name, params, body })
}

fn parse_struct_def(tokens: &mut TokenStream) -> PResult<Statement> {
    let span = tokens.expect(|t| matches!(t, TokenValue::Struct)).map_err(|_| ParseError::Continue)?.span;
...
fn parse_trait(tokens: &mut TokenStream) -> PResult<Statement> {
    let span = tokens.expect(|t| matches!(t, TokenValue::Trait)).map_err(|_| ParseError::Continue)?.span;
    let name = tokens.expect_identifier()?;
    let _ = tokens.expect(|t| matches!(t, TokenValue::LBrace))?;
    let methods = parse_methods(tokens);
    let _ = tokens.expect(|t| matches!(t, TokenValue::RBrace))?;

    Ok(Statement::Trait { span, name, methods })
}

fn parse_impl(tokens: &mut TokenStream) -> PResult<Statement> {
    let span = tokens.expect(|t| matches!(t, TokenValue::Impl)).map_err(|_| ParseError::Continue)?.span;
    let name = tokens.expect_identifier()?;
    match tokens.expect(|t| matches!(t, TokenValue::For | TokenValue::LBrace)) {
        Ok(Token { value: TokenValue::For, span }) => {
            let target = tokens.expect_identifier()?;
            Ok(Statement::ImplFor { span, name, target })
        },
        Ok(Token { value: TokenValue::LBrace, .. }) => {
            let methods = parse_methods(tokens);
            let _ = tokens.expect(|t| matches!(t, TokenValue::RBrace))?;
        
            Ok(Statement::Impl { span, target: name, methods })
        },
        Ok(_) => unreachable!(),
        Err(e) => Err(e)
    }
