...
use crate::slim::{execute_tree, get_parse_err_msg};
use std::cell::RefCell;
use std::collections::HashMap;
use std::fs;
use std::rc::Rc;

use crate::parser::{parse_ast, Block, Expr, ParseError, Span, Statement, Token, TokenValue};
...
pub fn evaluate_stmt(scope: &mut Scope, stm: &Statement) -> EResult<()> {
    match stm {
        ...
        Statement::Trait {
            ...
            name,
            methods,
        } => eval_declare_trait(scope, span, name, methods),
        Statement::Impl {
            span,
            target,
            methods,
        } => eval_impl(scope, span, target, methods),
    }
}
...
pub fn evaluate(scope: &mut Scope, expr: &Expr) -> EResult<ObjectRef> {
    match expr {
        ...
        Expr::Pipe {
            ...
            parent,
            child,
        } => eval_pipe(scope, span, parent, child),
        ...
        Expr::Func {
            ...
            params,
            body,
        } => eval_func(scope, span, name.as_deref(), params, body),
        Expr::Loop { span, body } => {
            match eval_while_with(
                scope,
                span,
        ...
        Expr::Conditional { span, branches } => {
            match eval_branches_with(scope, span, branches, &mut vec![])? {
                ...
                bubble => Err(rt_err!(
                    span,
                    "Cannot call {:?} outside a loop/function",
                    bubble
                )),
            }
        }
        ...
        Expr::Binary {
            ...
            left,
            right,
        } => eval_binary(scope, span, op, left, right),
        ...
        Expr::Assign {
            ...
            target,
            value,
        } => eval_assign(scope, span, target, value),
        _e => todo!(),
    }
}
...
fn eval_term(scope: &mut Scope, span: &Span, token: &TokenValue) -> EResult<ObjectRef> {
    match token {
        ...
        _t => Ok(ObjectRef::try_from(Token {
            span: span.to_owned(),
...
fn eval_self(scope: &mut Scope, span: &Span) -> EResult<ObjectRef> {
    scope.get("self").ok_or(RuntimeError(
        vec![span.to_owned()],
        "No self in scope".into(),
    ))
}

fn eval_struct(scope: &mut Scope, span: &Span, name: &str, props: &[Expr]) -> EResult<ObjectRef> {
    let rules = scope.get_struct_def(name).ok_or_else(|| {
        RuntimeError(
            vec![span.to_owned()],
...
fn eval_branches_with(
    ...
    for (cond, body) in branches.iter() {
        if evaluate(scope, cond)?.borrow().is_truthy() {
            ...
            for expr in body {
                match expr {
                    Block::Expression(Expr::Return(..))
                    | Block::Expression(Expr::Break(..))
                    | Block::Expression(Expr::Continue(..)) => {
                        return eval_interruptable_expr(
                            scope,
                            span,
                            &expr.clone().into_expr_checked(),
                            interrupts,
                        )
                    }
                    Block::Expression(Expr::Conditional { span, branches }) => {
...
fn eval_call_func(
    ...
    for expr in body {
        match expr {
            Block::Expression(Expr::Return(_span, e)) => {
                return evaluate(&mut inner, e).map_err(|e| e.add_span(span))
            }
            Block::Expression(Expr::Loop { span, body }) => match eval_while_with(
                &mut inner,
                span,
...
fn eval_unary(scope: &mut Scope, span: &Span, op: &TokenValue, value: &Expr) -> EResult<ObjectRef> {
    ...
    Ok(object.unary(op).map_err(|s| rt_err!(span, "{}", s))?.into())
}
    ...
    match target {
        ...
        Expr::Deref(span, ref inner) => match &**inner {
            Expr::Term(span, TokenValue::Identifier(name)) => {
                let prev = scope
                    .get(name)
                    .ok_or_else(|| rt_err!(span, "No variable named: {}", name))?;
